...
using UnityEngine.SceneManagement; // 确保此行存在且无拼写错误
...
namespace GlobalSnowEffect {
    ...
    public partial class GlobalSnow : MonoBehaviour {
        ...
        Material snowParticleMat, snowParticleIllumMat, snowParticleIllumOpaqueMat;
        ...
        void LoadResources() {
            ...
            if (depthShaderMask == null) {
                ...
                if (!depthShaderMask.isSupported) {
                    Debug.LogError("Depth Mask shader not supported on this platform.");
                    enabled = false;
                    return;
                }
            }
        ...
        void OnPreCull() {
            ...
            float brightness;
            if (_sun != null) {
                Shader.SetGlobalVector(ShaderParams.GlobalSunDir, new Vector4(_sun.transform.forward.x, _sun.transform.forward.y, _sun.transform.forward.z, _terrainMarksViewDistance * _terrainMarksViewDistance));
        ...
        private void OnPreRender() {
            if (cbMat != null && cameraEffect != null) {
                if (UnityEngine.XR.XRSettings.enabled) {
                    ...
                    cbMat.SetMatrix(ShaderParams.ClipToWorldMatrix, cameraEffect.cameraToWorldMatrix); // * cameraEffect.projectionMatrix.inverse);
                } else {
        ...
        void Update() {
            if (cameraEffect != null) {
                ...
                if (snowfallSystem != null) {
                    snowfallSystem.transform.position = GlobalSnow.instance.cameraEffect.transform.position + new Vector3(0, snowfallSystemAltitude, _snowfallWind * -50f);
                }

                if (snowdustSystem != null) {
                    snowdustSystem.transform.position = GlobalSnow.instance.cameraEffect.transform.position + new Vector3(0, -2 + _snowdustVerticalOffset, 0);
                }
        ...
        public void UpdateMaterialPropertiesNow() {
            ...
            if (deferred) {
                ...
                if (_distanceOptimization) {
                    ...
                    distantSnowMat.SetFloat(ShaderParams.Distance01, 0.000001f + (_detailDistance * 0.9f) / cameraEffect.farClipPlane);
                    distantSnowMat.SetFloat(ShaderParams.DistanceSlopeThreshold, 0.000001f + (_detailDistance * 0.9f) / cameraEffect.farClipPlane);
                }
        ...
        public void UpdateSnowdustProperties() {
            ...
            if (_snowdustIntensity > 0) {
                if (snowdustSystem == null) {
                    GameObject go = Instantiate(Resources.Load<GameObject>("Common/Prefabs/SnowDustSystem")) as GameObject;
        ...
        readonly Dictionary<GameObject, SnowColliderInfo> collisionCache = new Dictionary<GameObject, SnowColliderInfo>();
